...
def highlight_based_on_bits(
    ...
    active_color=(0.0, 0.0, 0.8, 0.5),
    inactive_color=(1.0, 0.0, 0.0, 0.5),
    ...
    and overlapping fragments. It also saves textual descriptions of highlighted atoms and bonds.
    ...
    Args:
        ...
        smarts_patterns (str | list | dict, optional):
            ...
            Defaults to the Klekota-Roth fingerprint SMARTS patterns.
        ...
        active_color (tuple[float, float, float, float], optional):
            RGBA color used to highlight active fragments. Defaults to blue (0.0, 0.0, 0.8, 0.5).
        inactive_color (tuple[float, float, float, float], optional):
            RGBA color used to highlight inactive fragments. Defaults to red (1.0, 0.0, 0.0, 0.5).
    ...
    Examples:
        ...
        highlight_based_on_bits(
            molecules="../ACE.sdf",
            output_dir="test_KR_not_providing_bits",
        )

    """

    kr_smarts_dict = {}
    default_features = KlekotaRothFingerprint(count=True).get_feature_names_out()
    for idx, feature in enumerate(default_features, start=1):
    ...
    if isinstance(smarts_patterns, str):
        if smarts_patterns.endswith(".json"):
            with open(smarts_patterns, "r") as f:
                raw = json.load(f)
            kr_smarts_dict = {}
            for key, smarts in raw.items():
                if key.startswith("KRFP"):
                    idx = int(key.replace("KRFP", ""))
                    kr_smarts_dict[idx] = smarts
            print("Loaded fingerprint dictionary " f"from JSON file: {smarts_patterns}")

    elif isinstance(smarts_patterns, list):
        kr_smarts_dict = {}
        for idx, feature in enumerate(smarts_patterns, start=1):
            kr_smarts_dict[idx] = feature
            print(kr_smarts_dict[idx])
    elif isinstance(smarts_patterns, dict):
        kr_smarts_dict = smarts_patterns

    max_bit = max(kr_smarts_dict.keys())
    smarts_list = [None] * (max_bit + 1)  # dummy index for 0

    for bit in range(1, max_bit + 1):
        smarts_list[bit] = kr_smarts_dict[bit]
    ...
    def visualize_fragments(
        ...
        drawer.drawOptions().legendFontSize = 30
